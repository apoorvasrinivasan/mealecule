...
public class CartMealeculeQuotientPopulator implements Populator<AbstractOrderModel, AbstractOrderData>
{
	...
	public void populate(final AbstractOrderModel source, final AbstractOrderData target)
	{
		...
		for (final AbstractOrderEntryModel entry : source.getEntries()) {
			...
			if (null != entry.getProduct().getMealeculeQuotient()) {
				...
				if (CollectionUtils.isNotEmpty(list)) {
					...
					totalCartCalories = totalCartCalories + list.stream().filter(mq -> MealeculeQuotientEnum.CALORIES.equals(mq.getMealeculeQuotientType())).findFirst().get().getValue() * productWeight / considerWeight * entry.getQuantity();
		...
		totalCartCalories = Double.valueOf(df.format(totalCartCalories));
		...
		if(CollectionUtils.isNotEmpty(source.getUser().getPreferredMealecule())){
			for (final String preferredMealecule:source.getUser().getPreferredMealecule()) {
				if(preferredMealecule.equalsIgnoreCase(MealeculeQuotientEnum.CARBOHYDRATE.getCode())){
					...
					mealeculeQuotientData.setCalories(totalCartCalories);
