			...
			final Collection<? extends CatalogOption> options, final PageOption page) throws ConversionException
	{
		...
		if (options.contains(CatalogOption.PRODUCTS))
		{
			...
			if(CollectionUtils.isNotEmpty(mealeculeQuotientDatas)) {
				...
				updateNutrientFacet(facetDataList, MealeculeQuotientEnum.CALORIES, mealeculeQuotientDatas.stream().sorted(Comparator.comparingDouble(MealeculeQuotientData::getCalories).reversed()).findFirst().get().getCalories(), mealeculeQuotientDatas.stream().sorted(Comparator.comparingDouble(MealeculeQuotientData::getCalories)).findFirst().get().getCalories());
